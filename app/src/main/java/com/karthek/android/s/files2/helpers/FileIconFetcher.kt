...
class FileIconFetcher(private val fileType: FileType) : Fetcher<SFile> {
        ...
        val bitmap = runCatching {
            when {
                type.startsWith("image/") -> {
                    createImageThumbnail(data.file, pixelSize.width, pixelSize.height, type)
                }
                type.startsWith("audio/") -> {
                    createAudioThumbnail(data.file, pixelSize.width, pixelSize.height)
                }
                type.startsWith("video/") -> {
                    createVideoThumbnail(data.file, pixelSize.width, pixelSize.height)
                }
                ...
                type == "application/pdf" -> {
                    getPdfThumbnail(data.file, pixelSize.width, pixelSize.height, pool)
                }
                else -> {
                    throw Exception()
                }
    ...
    override fun key(data: SFile): String {
